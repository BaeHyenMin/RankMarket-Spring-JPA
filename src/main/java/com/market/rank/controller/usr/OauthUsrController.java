...
OauthUsrController {
    ...
    public ResponseEntity<ResDto> mobileLogin(@RequestBody Map<String, String> payload) {
        ...
        Optional<Usr> usr = usrViewService.searchUsrInfo(payload.get("mail"));
        ReqLoginUsrDto reqLoginUsrDto;
        if(usr.isPresent()){

            usrProcService.deviceTokenSave(payload.get("token"), usr.get());
            reqLoginUsrDto = ReqLoginUsrDto
                    .builder()
                    .usrId(usr.get().getUsrId())
                    .usrNm(usr.get().getUsrNm())
                    .build();
        }else{
            reqLoginUsrDto = ReqLoginUsrDto
                    .builder()
